// VENDORED VERBATIM from main branch `react/atom/editor.tsx` (v1 rich-text
// editor) so the component preview exercises ALL real v1 features (table,
// image, youtube, link, color, lists, …). The only change vs source is the
// styles import: v1 used a CSS-module; here `styles.x` returns the plain class
// name `x`, which matches the scoped v1 CSS in v1-components.generated.css when
// rendered inside `.podo-v1-stage`. Do not hand-edit; re-vendor from main.
// @ts-nocheck
/* eslint-disable */
...
// `ownKeys`/`getOwnPropertyDescriptor` expose the paragraph-style keys so the
// toolbar's `Object.keys(styles)` paragraph-style detection still works.
...
const Editor = ({
  ...
  const t = useT();
  const resolvedPlaceholder = placeholder ?? t('v1Editor.placeholder');
  ...
  // 정렬 옵션 정의
  const alignOptions = [
    { value: 'left', label: t('v1Editor.alignLeft'), icon: 'alignLeft' },
    { value: 'center', label: t('v1Editor.alignCenter'), icon: 'alignCenter' },
    { value: 'right', label: t('v1Editor.alignRight'), icon: 'alignRight' },
  ];

  // 문단 형식 옵션 정의
  const paragraphOptions = [
    { value: 'h1', label: t('v1Editor.paraH1') },
    { value: 'h2', label: t('v1Editor.paraH2') },
    { value: 'h3', label: t('v1Editor.paraH3') },
    { value: 'p', label: t('v1Editor.paraBody'), className: styles.pDefault },
    { value: 'p1', label: t('v1Editor.paraP1'), className: styles.p1Preview },
    { value: 'p2', label: t('v1Editor.paraP2'), className: styles.p2Preview },
    { value: 'p3', label: t('v1Editor.paraP3'), className: styles.p3Preview },
    { value: 'p3_semibold', label: t('v1Editor.paraP3Semibold'), className: styles.p3_semiboldPreview },
    { value: 'p4', label: t('v1Editor.paraP4'), className: styles.p4Preview },
    { value: 'p4_semibold', label: t('v1Editor.paraP4Semibold'), className: styles.p4_semiboldPreview },
    { value: 'p5', label: t('v1Editor.paraP5'), className: styles.p5Preview },
    { value: 'p5_semibold', label: t('v1Editor.paraP5Semibold'), className: styles.p5_semiboldPreview },
  ];

  // 현재 선택된 스타일의 라벨 가져오기
  const getCurrentStyleLabel = () => {
    const option = paragraphOptions.find(opt => opt.value === currentParagraphStyle);
    return option ? option.label : t('v1Editor.paraFormat');
  };

  // 현재 정렬 상태의 라벨 가져오기
  const getCurrentAlignLabel = () => {
    const option = alignOptions.find(opt => opt.value === currentAlign);
    return option ? option.label : t('v1Editor.alignLeft');
  };
  ...
  const insertImage = async () => {
    ...
    // URL이 입력된 경우
      ...
      try {
        ...
        alert(t('v1Editor.alertImageLoadFailedUrl', { url: imageUrl }));
        return;
      }
    }
    ...
    // 이미지 로드 에러 처리
    img.onerror = () => {
      alert(t('v1Editor.alertImageLoadFailed', { src: imageSrc }));
  ...
  const deleteTableRow = () => {
    ...
    if (tbody.rows.length <= 1) {
      alert(t('v1Editor.alertTableMinRow'));
      return;
    }
  ...
  const deleteTableColumn = () => {
    ...
    if (row.cells.length <= 1) {
      alert(t('v1Editor.alertTableMinColumn'));
      return;
    }
  ...
  const insertYoutube = () => {
    ...
    if (!videoId) {
      alert(t('v1Editor.alertInvalidYoutubeUrl'));
      return;
    }
  ...
  const handleImageFileSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file && file.type.startsWith('image/')) {
      ...
      alert(t('v1Editor.alertSelectImageFile'));
    }
    ...
    <div className={`${styles.editor} ${statusClass}`} style={{ width, position: 'relative' }}>
              ...
              >
                <div className={styles.tableGridSelector}>
                  <div className={styles.tableGridLabel}>
                    {tableRows > 0 && tableCols > 0 ? t('v1Editor.tableGridSize', { rows: tableRows, cols: tableCols }) : t('v1Editor.tableSelectSize')}
                  </div>
            ...
            {isLinkDropdownOpen && (
              ...
              >
                <div className={styles.linkInput}>
                  <label>{t('v1Editor.urlLabel')}</label>
                  <input
                    type="text"
                    value={linkUrl}
                ...
                <div className={styles.linkTarget}>
                  ...
                  <label>
                    <input
                      ...
                      onChange={(e) => setLinkTarget(e.target.value)}
                    />
                    {t('v1Editor.openInCurrentWindow')}
                  </label>
                </div>
              ...
              >
                <div className={styles.imageTabSection}>
                  ...
                  {imageTabMode === 'file' && (
                    <div className={styles.imageFileSection}>
                      ...
                      >
                        {imageFile ? imageFile.name : t('v1Editor.selectFile')}
                      </button>
                      {imagePreview && (
                        <div className={styles.imagePreviewBox}>
                          <img src={imagePreview} alt={t('v1Editor.imagePreviewAlt')} />
                        </div>
                      )}
                    </div>
                  )}
                ...
                <div className={styles.imageOptions}>
                  <div className={styles.imageOptionRow}>
                    <label>{t('v1Editor.sizeLabel')}</label>
                    <div className={styles.imageSizeButtons}>
                      ...
                      >
                        {t('v1Editor.sizeOriginal')}
                      </button>
                    </div>
                  </div>

                  <div className={styles.imageOptionRow}>
                    <label>{t('v1Editor.alignLabel')}</label>
                    <div className={styles.imageAlignButtons}>
                      <button
                        type="button"
                        className={imageAlign === 'left' ? styles.active : ''}
                        onClick={() => setImageAlign('left')}
                        title={t('v1Editor.alignLeft')}
                      >
                  ...
                  <div className={styles.imageOptionRow}>
                    <label>{t('v1Editor.altTextLabel')}</label>
                    <input
                      type="text"
                      value={imageAlt}
                      onChange={(e) => setImageAlt(e.target.value)}
                      placeholder={t('v1Editor.imageDescriptionPlaceholder')}
                    />
                  </div>
                </div>
                ...
                <div className={styles.imageTabSection}>
                  ...
                  <div className={styles.imageUrlSection}>
                    <input
                      ...
                      placeholder={t('v1Editor.youtubeUrlPlaceholder')}
                      autoFocus
                    />
                  </div>
                </div>
        ...
        {isToolbarItemEnabled('code') && (
          <div className={styles.toolbarGroup}>
            <button
              ...
              title={isCodeView ? t('v1Editor.switchToEditor') : t('v1Editor.viewHtmlCode')}
            >
        ...
        >
          <div className={styles.editLinkContent}>
            <div className={styles.editLinkInput}>
              <label>{t('v1Editor.editUrlLabel')}</label>
              <input
                type="text"
                value={editLinkUrl}
      ...
      {isImageEditPopupOpen && selectedImage && (() => {
        ...
        return (
          ...
          >
          <h3 style={{ margin: '0 0 15px 0', fontSize: '16px', fontWeight: '600' }}>{t('v1Editor.editImage')}</h3>
      ...
      {isYoutubeEditPopupOpen && selectedYoutube && (() => {
        ...
        return (
          ...
          >
            <h3 style={{ margin: '0 0 15px 0', fontSize: '16px', fontWeight: '600' }}>{t('v1Editor.editYoutube')}</h3>
        ...
        >
          {selectedTableCells.length > 1 && (
            <div className={styles.tableContextMenuHeader}>
              {t('v1Editor.cellsSelected', { count: selectedTableCells.length })}
            </div>
          )}
          ...
          <div className={styles.tableContextMenuItem}>
            ...
            >
              {selectedTableCells.length > 1 ? t('v1Editor.cellBackgroundColorCount', { count: selectedTableCells.length }) : t('v1Editor.cellBackgroundColor')}
          ...
          <button
            ...
            className={styles.tableContextMenuButton}
          >
            {t('v1Editor.resetBackgroundColor')}
          </button>

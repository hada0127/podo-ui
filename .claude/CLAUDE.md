...
- **브라우저 테스트**: 브라우저 테스트가 필요할 때는 Playwright 대신 Claude in Chrome 연동 기능(`mcp__claude-in-chrome__*` 도구들)을 사용할 것
...
**모든 컴포넌트/시스템 변경 작업 후 반드시 `ai-json-sync` 에이전트를 실행하여 `public/ai/` JSON을 최신 상태로 유지해야 합니다.**
이 데이터는 외부 프로젝트에서 podo-ui MCP 서버의 데이터 소스로 사용되므로, 동기화가 누락되면 외부 AI 도구가 잘못된 정보를 참조하게 됩니다.
